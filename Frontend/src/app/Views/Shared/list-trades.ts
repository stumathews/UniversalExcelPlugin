...
import { Trade,  GetPortfolioTradesResponse} from '@finbourne/lusidtypes';
...
export class ListTradesComponent implements OnInit {
  ...
  readyTradeForUpsert(thinTrade: Trade) {
    const upsertTrade = thinTrade;
    upsertTrade.properties = [];
    upsertTrade.tradeId = upsertTrade.tradeId.length === 0 ? DateUtils.GetTodaysDate() + upsertTrade.nettingSet + upsertTrade.currency : upsertTrade.tradeId; // must have one else breaks server 
